...
| `--orange` | `oklch(0.55 0.19 45)` في الوضعين | مرادف `--primary` بصيغة Smart-Menu — مؤشرات، أيقونات نشطة، حدود برتقالية بأصناف `*-orange/ن` |
| `--accent` | `oklch(0.55 0.19 45 / 0.15)` داكن / `/ 0.12` فاتح | **تلميح خافت** فقط — `hover:bg-accent` و`tint` ناعم. ليس لونًا صلبًا أبدًا |
| `--orange-muted` | `oklch(0.55 0.19 45 / 0.15)` داكن / `/ 0.12` فاتح | تلميح برتقالي بصيغة Smart-Menu (مرادف `--accent`) |
...
**قاعدة الاشتقاق (محدّثة):** `--primary`/`--orange`/`--ring` قيم صلبة **مباشرة** (لا `var(--accent)`)
لتطابق بنية Smart-Menu حرفيًا. `--accent`/`--accent-soft`/`--orange-muted` تلميحات خافتة
متطابقة القيمة. أي برتقالي صلب جديد يستخدم `--primary` أو `--orange`، أي تلميح ناعم
يستخدم `--accent` أو `--orange-muted`.

**الوضع الفاتح:** يفرض الوضع الفاتح قيمًا أغمق للـ AA على الأبيض (primary = 0.40،
orange يبقى 0.55 — «السطوع المزدوج» حرفيًا كما في Smart-Menu؛ success = 0.48،
