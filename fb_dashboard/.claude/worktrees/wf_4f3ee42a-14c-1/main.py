...
async def reply_to_conversation(conversation_id: str, message: str = Form(...),
    ...
    if not senders:
        raise HTTPException(400, "No senders found in conversation")
    target = senders[0].get("id", "")
    if not target:
        raise HTTPException(400, "No sender ID found")
    result = await fb.send_dm(target, message)
    if not result:
        raise HTTPException(400, "Failed to send reply")
    return {"ok": True}
