    ...
    async def cycle(self):
        ...
        async with AsyncSessionLocal() as session:
            try:
                ...
                posts = await self.fb.get_page_posts(10)
...
# ---------------------------------------------------------------------------
# Legacy entry points (keep interface for main.py compatibility)
# ---------------------------------------------------------------------------

_shared_engine: BotEngine | None = None
_cycle_counter = 0


async def run_auto_reply(fb: FBClient):
    """Legacy entry point — delegates to BotEngine."""
    global _shared_engine
    if _shared_engine is None:
        _shared_engine = BotEngine(fb)
    await _shared_engine.cycle()


async def _process_comment(fb: FBClient, session, c: dict, pid: str,
                           replied_ids: set, rules_data: list[dict],
                           welcomed_senders: set[str], dm_map: dict[str,str] | None = None) -> None:
    """Legacy pipeline interface for webhook direct calls."""
    matcher = RuleMatcher(rules_data, dm_map or {})
    dedup = DedupEngine()
    dedup.load_from_db(replied_ids)
    cooldown = CooldownManager()
    pipeline = ReplyPipeline(fb, dedup, cooldown)
    await pipeline.process(session, c, pid, matcher)


async def _load_rules(session) -> list[dict]:
    """Legacy — re-exports BotEngine._load_rules."""
    engine = BotEngine(None)
    return await engine._load_rules(session)


async def load_replied_ids(session) -> set:
    """Legacy — re-exports BotEngine._load_replied_ids."""
    engine = BotEngine(None)
    return await engine._load_replied_ids(session)


async def import_json_data(session, rules_file: str, state_file: str):
    """Import rules from JSON file on first run."""
    count = await session.scalar(select(func.count(Rule.id)))
    if count == 0:
...
async def bot_worker():
    ...
    from pathlib import Path
    log.info("FB Auto-Reply Bot started (standalone)")
