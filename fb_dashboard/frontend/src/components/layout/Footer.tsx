        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8 mb-10 sm:mb-12">
          <div className="col-span-2 sm:col-span-1">
            ...
            <div className="flex gap-2">
              {/* v9-D1: role="img" legalizes aria-label on a span (axe
                  aria-prohibited-attr — WCAG A) — the icon box IS the image. */}
              <span role="img" aria-label="قريباً" className="size-8 rounded-sm bg-card border border-border flex items-center justify-center opacity-40 cursor-not-allowed">
                <svg className="size-3.5" aria-hidden="true" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
              </span>
              <span role="img" aria-label="قريباً" className="size-8 rounded-sm bg-card border border-border flex items-center justify-center opacity-40 cursor-not-allowed">
              ...
              <a href={`https://wa.me/${waNumber}`} className="size-8 rounded-sm bg-card border border-border flex items-center justify-center hover:bg-accent-foreground/20 transition-colors" aria-label="واتساب"><MessageCircle className="size-3.5" /></a>
            </div>
          </div>
            ...
            <div className="space-y-2.5 text-sm text-muted-foreground">
              {SERVICES.map((svc, i) => (
                svc.href ? <Link key={i} href={svc.href} className="block w-fit hover:text-foreground transition-colors">{svc.label}</Link>
                  : <span key={i} className="block w-fit cursor-default">{svc.label}</span>
              ))}
            </div>
          </div>
