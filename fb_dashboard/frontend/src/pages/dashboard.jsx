...
export function Dashboard(_p) {
  ...
  return (
    <section className="page active" dir="rtl">
      ...
      <div className="stats-grid">
        {[
          { icon: statIcons.total, color: "", label: "آخر 7 أيام", val: stats?.total_replies || 0, trend: stats?.trend?.week, arrow: (stats?.trend?.week ?? 0) >= 0 ? "↑" : "↓" },
          { icon: statIcons.today, color: "success", label: "ردود اليوم", val: stats?.today_replies || 0, trend: stats?.trend?.today, arrow: (stats?.trend?.today ?? 0) >= 0 ? "↑" : "↓" },
