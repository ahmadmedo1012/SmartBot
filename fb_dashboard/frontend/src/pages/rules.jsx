    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit({ ...initial, name, keywords, reply_template: reply, description: desc, bot_type: botType, dm_template: dmTemplate }) }}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-muted-foreground">الأصغر = الأعلى أولوية</p>
        </div>
      </div>
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-foreground flex items-center gap-1.5">
      ...
      <div className="space-y-1.5">
        ...
        <p className="text-xs text-muted-foreground">المتغيرات: {`{name}`} {`{mention}`} {`{message}`}</p>
      </div>
      <div className="space-y-1.5">
        <label className="text-sm font-medium text-foreground flex items-center gap-1.5">
          <MessageSquare className="size-3.5 text-muted-foreground" />رسالة خاصة <Badge className="text-xs px-1.5 py-0">اختياري</Badge>
        </label>
        <Textarea value={dmTemplate} onChange={(e) => setDmTemplate(e.target.value)} rows={2} placeholder={"أهلاً {name}! شكراً لتواصلك 💬"} />
        <p className="text-xs text-muted-foreground">تُرسل للمستخدمين النشطين عبر Messenger. المتغيرات: {`{name}`} {`{mention}`}</p>
      </div>
      <div className="space-y-1.5">
                  ...
                  <tr key={r.id} className="hover:bg-accent/20 transition-colors">
                    <td className="px-4 py-4 align-middle">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge className={cn(r.bot_type === "welcome" ? "bg-info/15 text-info border-info/20 text-xs" : "bg-primary/10 text-primary border-primary/20 text-xs", "rounded-full px-2 py-0")}>
                    ...
                    <td className="px-4 py-4 align-middle">
                      <div className="flex flex-wrap gap-1 max-w-[200px]">
                        {r.keywords?.slice(0, 5).map((kw, i) => (
                          <span key={i} className="inline-flex text-xs px-1.5 py-0.5 rounded-md bg-muted text-muted-foreground">
                            {kw.length > 12 ? kw.slice(0, 12) + "…" : kw}
                          </span>
                        ))}
                        {r.keywords?.length > 5 && <span className="text-xs text-muted-foreground">+{r.keywords.length - 5}</span>}
                      </div>
                    </td>
