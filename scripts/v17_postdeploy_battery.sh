#!/usr/bin/env bash
# v17 post-deploy battery — live production verification (stateless, ~100 checks)
# v17-E6 (خطة v17 §E6-م6 + D3) — نمط v15_postdeploy_battery.sh حرفياً + القسم P:
...
#   P canary browser pass (جديد v17 — dec-canary-watch): Playwright يحمّل
...
#
# Usage:  bash scripts/v17_postdeploy_battery.sh
# Env:    BASE_URL_WEB (default https://bot.smart-link.ly)
#         BASE_URL_API (default https://api.smart-link.ly)
#         V17_LIGHT=0  — disable login-empty-body check
#         CANARY_PAGES (default "/,/pricing,/login,/dashboard")
# Output: docs/evidence/v17/post-deploy-verification.txt
...
EVID="$ROOT_DIR/docs/evidence/v17"
mkdir -p "$EVID"
...
echo "== v17 post-deploy battery — $(date -u '+%Y-%m-%dT%H:%M:%SZ') ==" | tee "$REPORT"
...
curl -s "$BOT/v17-nonexistent" > "$T/nf1.html"
contains "A5 skip-link on 404 (bot domain)" "$T/nf1.html" 'id="page-content"'
curl -s "$API/v17-nonexistent" > "$T/nf2.html"
...
# v17 (E2 أزال مسار ?token= — dec-cron-restore): الاستعلام صار يُرفض كأي
# نداء بلا ترويسة — العقد الآن: 401/403 (الترويسة فقط)
C=$(curl -s -o /dev/null -w "%{http_code}" "$API/api/cron/heartbeat?token=x")
statusIn "K69b heartbeat ?token= مُزال → 401/403 (v17)" "$C" "401,403"
...
# ══ P. canary browser pass — أخطاء console/وعود غير معالجة (v17 الجديد) ═══
...
// v17-E6 (dec-canary-watch) — canary: console errors + unhandled rejections
...
try {
  ...
  if (!baseline || !baseline.pages) {
    ...
    const seeded = {
      $schema: 'console-canary-baseline-v17',
      comment: 'v17-E6 (dec-canary-watch) — بصمة أخطاء console/الوعود غير المعالجة للصفحات العامة. تُبذّر بأول تشغيل للبطارية ثم أي قياس لاحق يجب ألا يتجاوزها (≤ لكل صفحة وإجمالاً). تحديثها قرار منسّق مقصود مع تبرير — لا آلية صامتة.',
...
# ══ Q. v17 feature truths (read-only — ميزات الجولة حية) ════════════════════
# Q92: /subscribe للمجهول — 200 (كان يطرد لlogin قبل v17-E-F2)
C=$(curl -s -o /dev/null -w "%{http_code}" "$BOT/subscribe"); check "Q92 /subscribe anonymous 200 (v17: skipAuthRedirect)" "200" "$C"
curl -s "$BOT/subscribe" > "$T/sub.html"
contains "Q93 /subscribe يعرض الخطط للمجهول" "$T/sub.html" 'الباقات\|الخطط\|اشترك'
# Q94: عنصر تنقل التسلسلية موجود في صفحة dashboard (ملف تعريف HTML المصدر)
curl -s "$BOT/dashboard/sequences" > "$T/seq.html"
C=$(curl -s -o /dev/null -w "%{http_code}" "$BOT/dashboard/sequences")
statusIn "Q94 /dashboard/sequences route exists (redirects to login when anonymous)" "$C" "200,307,308"
# Q95: الأدمن-سوبورت حي
C=$(curl -s -o /dev/null -w "%{http_code}" "$BOT/admin/support")
statusIn "Q95 /admin/support route exists" "$C" "200,307,308"
# Q96: login فارغ الجسم → 400 (لا 500 — عائلة v15)
C=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$API/api/login" -H 'Content-Type: application/json' -d '{}')
statusIn "Q96 login empty body 400 (no 500)" "$C" "400,422"
# Q97: mark-read endpoint موجود (401 بدون مصادقة — ليس 404)
C=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$API/api/inbox/conversations/w_x/read")
statusIn "Q97 mark-read endpoint live (401 anonymous — not 404)" "$C" "401,403"
