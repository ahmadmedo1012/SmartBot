#!/usr/bin/env bash
# v17-E6 (تصميم D3 §6) — بطارية المحاكاة المحلية v17 الكاملة (14 شخصية)
#
# الوظيفة (من نسخة v15-E8 حرفياً): يقلع الخادمين (uvicorn + next start
# بوكيل) على قاعدة SQLite نظيفة (v17-sim.db) ثم يشغّل بطارية Playwright
# كاملة (p01→p14) ثم يطفئ الخوادم نظيفاً — مع تقرير عربي لكل شخصية +
# **خروج مركّب صادق**: exit = playwright_exit || claims_exit.
#
# جديد v17 على نمط v15 (صدق البطارية — إصلاحا D3):
...
#   2. SIM_ROUND تصدَّر فعلياً: من scripts/round.env (ROUND=v17 — يبذره
#      E7) وإلا v17 صريحة. db-claims.mjs يقرؤها فيعضّ TTL إدخالات
#      expires (كان عالقاً في v15: 10/12 إدخالات منتهية ظلت تُطبَّق).
#   3. قائمة السماح (sim-findings.json) مقلَّمة v17-E6: 6 إدخالات —
#      3 عائلة R3 بexpires=prod (قياس إنتاج بتوكن Graph حي) + 3 إدخالات
#      expires=v17 «أثبت-ثم-احذف» (p13-ssrf-receipt-dns/p12-wizard-focus/
...
# وscripts/v15_sim_rotate_secret.sh يقرآنها حرفياً (قيمها v17).
#
# التشغيل من جذر المستودع:
#   bash scripts/v17_sim_local_battery.sh
# متغيرات:
#   SIM_STRICT_409=0        — تليين قفل p06 (تشخيص فقط)
#   SIM_ROUND               — تُحترم إن صُدِّرت خارجياً (وإلا round.env/v17)
#   V17_SIM_FRONT_PORT      — منفذ الواجهة (افتراضي 3200)
#   V17_SIM_API_PORT        — منفذ الخلفية (افتراضي 8000)
...
# round.env (E7) هو المصدر الوحيد للجولة عند الدمج؛ قبل وجوده نثبت v17
...
# ── 2) أسرار الجلسة (SK/FK/CS → /tmp/v17-backend.env) ─────────────────────
...
if ! curl -sf "http://127.0.0.1:$API_PORT/healthz" > /dev/null 2>&1; then
  echo "فشل إقلاع الخلفية — آخر سجل:"; tail -20 /tmp/v17-uvicorn.log
  exit 2
fi
...
if [ ! -d "$HOME/.cache/ms-playwright" ] || [ -z "$(ls -d "$HOME/.cache/ms-playwright"/chromium_headless_shell-* 2>/dev/null | head -1)" ]; then
  ...
  npx playwright install chromium > /tmp/v17-pw-install.log 2>&1 || {
    echo "فشل تثبيت المتصفحات:"; tail -10 /tmp/v17-pw-install.log; exit 2; }
fi

start_front() {
  LOCAL_API_PROXY="http://127.0.0.1:$API_PORT" setsid nohup npx next start -p "$FRONT_PORT" \
    > /tmp/v17-next.log 2>&1 < /dev/null &
  disown
...
if ! probe_ok; then
  ...
  if [ "${SKIP_FRONT_BUILD:-0}" != "1" ]; then
    LOCAL_API_PROXY="http://127.0.0.1:$API_PORT" npm run build > /tmp/v17-next-build.log 2>&1 || {
      echo "فشل إعادة البناء:"; tail -25 /tmp/v17-next-build.log; pkill -f "uvicorn runner:app"; exit 2; }
  fi
  start_front
fi
if ! probe_ok; then
...
echo "── البطارية v17 تبدأ (RUN=$RUN_STAMP · SIM_ROUND=$SIM_ROUND) — 14 شخصية ──"
...
{
  echo "══════════════════════════════════════════════════════════"
  echo "تقرير بطارية المحاكاة المحلية v17 (E6 — إصلاحا D3 لصدق البطارية)"
  echo "══════════════════════════════════════════════════════════"
  echo "التاريخ: $(date -u '+%Y-%m-%dT%H:%M:%SZ')"
  echo "الجولة (RUN): $RUN_STAMP · SIM_ROUND=$SIM_ROUND (round.env: $([ -f "$ROOT/scripts/round.env" ] && echo نعم || echo غائب — افتراضي v17))"
  ...
  echo ""
  echo "── كاشف الانقلاب (status=findingClosed — مُصلَح v17) ──"
  LIVE="$SIM_ART/$RUN_STAMP/sim-findings-live.json"
  if [ -f "$LIVE" ]; then
    # v17-fix (D3 #1): db-claims.mjs يكتب في الملف الحي سطور
  ...
  if [ "$PW_EXIT" -eq 0 ] && [ "$CLAIMS_EXIT" -eq 0 ]; then
    ...
    echo "  فشل — راجع /tmp/v17-playwright.log و $RUN_DIR — FAIL"
  fi
} | tee "$REPORT"
